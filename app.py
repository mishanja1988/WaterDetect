...
def _render_plot_image(kind: str, g: pd.DataFrame, well: str) -> BytesIO:
    """Сгенерировать matplotlib-изображение графика в память (PNG)."""
    buf = BytesIO()
    if kind == "MG":
        ...
        ax.plot(g.loc[m_wor, "t_pos"], g.loc[m_wor, "WOR"],
                marker="o", linestyle="none", markersize=4, label="WOR")
...
def export_with_charts_per_well(df: pd.DataFrame, kind: str) -> BytesIO:
    """
    Создать Excel-файл в памяти: на каждый well — отдельный лист:
    таблица в колонках A:..., график вставлен справа (примерно в колонку J).
    kind: 'MG' или 'Chan'
    """
    output = BytesIO()
    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
        for well, g in df.groupby("well", sort=False):
            ...
            g_to_write = g.copy()
            # Сбросить индексы, чтобы таблица начиналась с A1
            g_to_write.reset_index(drop=True, inplace=True)
            ...
            # Вставим картинку справа от таблицы (рядом): в ячейку J2 (колонка 10, строка 1)
            ws.insert_image(1, 9, f"{kind}_{well}.png", {"image_data": img, "x_scale": 1.0, "y_scale": 1.0})
    output.seek(0)
    return output
...
def upload_result(df_MG, df_Chan):
    ...
    if (but_excel_MG and mg_bytes) or (but_excel_Chan and ch_bytes):
        st.success("Таблицы с графиками успешно сохранены.")
